...
(Step 5, not built yet), which will wrap both of them in a pure
`step(state, event) -> (new_state, actions)` function that `api.py` (Step
7) exposes over HTTP, and that either `sim.py` (a terminal fake, Step 8) or
a real Twilio call (Step 10+, later) can drive identically.
...
flowchart TB
    ...
    subgraph menu_layer["menu.py — Step 6, NOT BUILT YET"]
        ...
        MENU -.->|will call| ENGINE
    end

    subgraph surface["api.py — Step 7, NOT BUILT YET"]
        API["api.py (FastAPI)<br/>POST /call/start<br/>POST /call/event<br/>GET /call/id/state"]
        ENGINE -.->|wrapped by| API
    end

    subgraph clients["Two interchangeable clients of the same API"]
        SIM["sim.py — Step 8, NOT BUILT YET<br/>terminal keypress simulator"]
        TWILIO["Twilio webhook — Step 10+, FUTURE<br/>real phone calls"]
        API -.-> SIM
        API -.-> TWILIO
    end
...
Nothing external is required for the system to work right now, and that stays true through Step 8. See the one-liner roadmap below for exactly where each one plugs in.
